      ...
      <div id="modalFoto" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.7);z-index:9999;align-items:center;justify-content:center">
        <div style="background:var(--negro-card);border-radius:12px;padding:24px;width:90%;max-width:540px;max-height:90vh;overflow-y:auto">
          <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
            <h3 style="margin:0">📷 Importar Box Score por Foto</h3>
            <button onclick="cerrarModalFoto()" style="background:none;border:none;color:var(--gris);font-size:20px;cursor:pointer">✕</button>
          </div>
          <p style="color:var(--gris);font-size:13px;margin-bottom:16px">Toma una foto del box score oficial o sube una imagen. La IA extraerá las estadísticas automáticamente.</p>
...
async function analizarFoto() {
  ...
  resultDiv.innerHTML = '<p style="color:var(--gris);text-align:center">🤖 Analizando imagen con IA...</p>';
  ...
  try {
    ...
    if (res.stats && res.stats.length > 0) {
      ...
      resultDiv.innerHTML = '<p style="color:var(--rojo)">No se pudieron extraer estadísticas. Intenta con una imagen más clara.</p>';
    }
  } catch (err) {
...
function renderTablaFoto(res) {
  const resultDiv = document.getElementById('resultadoFoto');
  let html = `
    <h4 style="margin-bottom:12px;color:var(--rojo)">Estadísticas extraídas — revisa y aplica</h4>
...
async function cargarDatosPartido(partidoId) {
  ...
  try {
    ...
    if (p.stats_rival) {
      const sr = p.stats_rival;
      const map2 = {puntos:'puntos',rebotes:'rebotes',asistencias:'volumen',robos:'robos',
        bloqueos:'wbloqueos',perdidas:'wperdidas',faltas:'faltas',
        tci:'tiros_campo_intentados',tce:'tiros_campo_encestados',
        t3i:'tiros_3_intentados',t3e:'tiros_3_encestados',
        tli:'tiros_libres_intentados',tle:'tiros_libres_encestados'};
      for (const [key, field] of Object.entries(map2)) {
...
function renderTablaFoto(res) {
  ...
  let html = `
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px">
      <thead><tr style="background:var(--negro-hover)">
        <th style="padding:6px;text-align:left">Jugador</th>
        <th>MIN</th><th>PTSe/th><th>REB</th><th>AST</th><th>ROB</th><th>BLqst/th>
      </tr></thead><tbody>`;
...
function aplicarStatsFoto() {
  ...
  cerrarModalFoto();
  showToast('✓ Estadísticas aplicadas — revisa y guarda');
}
